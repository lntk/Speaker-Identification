...
<DeleteSpeaker>:
    ...
    BoxLayout:
        orientation: 'vertical'
        Label:
            text: 'Enter your speaker you dont want to see again'
        TextInput:
            id: delete_name
            text: ''
            on_text: root.update_padding(args[0])
...
class TestPlayerScreen(Screen):
    ...
    def start(self):
        ...
        audio_thread = threading.Thread(target=self.plays, args=())
        audio_thread.start()
        test_thread = threading.Thread(target=si.testAudio, args=(audio_path,))
        result_thread = threading.Thread(target=self.display_result, args=(test_thread,))
        test_thread.start()
        result_thread.start()
    ...
    def display_result(self, test_thread): 
        while True:
            if not test_thread.isAlive():
                IdentifyResult(si.identified_speaker).open()
                break
...
class ListSpeakers(Popup):
    ...
    def __init__(self, **kwargs):
        ...
        list_speakers = si.load_speaker().keys()
...
class DeleteSpeaker(Popup):
    def delete_speaker(self):
        speaker_name = self.ids['delete_name'].text
        si.delete_speaker(speaker_name)
        self.open_list_speakers()
...
class EnrollRecorder(AudioTool):
    ...
    def startRecording_clock(self):
        ...
        enroll_thread = threading.Thread(target=si.enrollName, args=(self.speaker_name, self.duration,))
        result_thread = threading.Thread(target=self.display_result, args=(enroll_thread,))
        enroll_thread.start()
        result_thread.start()

    def display_result(self, enroll_thread): 
        while True:
            if not enroll_thread.isAlive():
...
class IdentifyRecorder(AudioTool):
    ...
    def startRecording_clock(self):
        ...
        identify_thread = threading.Thread(target=si.identify, args=())
        result_thread = threading.Thread(target=self.display_result, args=(identify_thread,))
        identify_thread.start()
        result_thread.start()

    def display_result(self, identify_thread): 
        while True:
            if not identify_thread.isAlive():
                IdentifyResult(speaker_name=si.identified_speaker).open()
                break
...
class SpeakerIdentificationApp(App):
    def build(self):
        return screen_manager

sample_app = SpeakerIdentificationApp()
sample_app.run()